function Meiosis_draw(gamma)
    ...
    [Base,  Base_f]    = stlread('Base.stl');
    [Link1, Link1_f]   = stlread('Link1.stl');
    [Link2, Link2_f]   = stlread('Link2.stl');
    [Link3, Link3_f]   = stlread('Link3.stl');
    [Link4, Link4_f]   = stlread('Link4.stl');
    [Link5, Link5_f]   = stlread('Link5.stl');
    [Link6, Link6_f]   = stlread('Link6.stl');

    %Relative Positions:
    rBfromI = [0;       0;       0];
    r1fromB = [0;       0;    8.75]; % base is 8.75 cm tall
    r2from1 = [0;    -1.54;  13.33]; % link 1 is 13.33 tall and displaced by 1.54 cm in the neg. y-direction
