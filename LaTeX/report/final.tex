...
The third joint’s torque requirements can be approximated in the same manner. However, links three, four, and five’s masses are assumed to act 200 mm from the third joint. \emph{Table \ref{tab:arm2}} estimates the links’ masses.
...
&\multirow{3}{*}{\textbf{Mass (kg)}}& \textbf{CM from Rotaion (m)} &\textbf{Max Torque (Nm)}& \textbf{Required Servo Torque (Nm)} & \textbf{MX-12W Delivered Torque (Nm)}  \\ \hline
...
The manipulator has a reachable workspace of 588.25 mm. This workspace is within the specified workspace of 300-700 mm. Within the reachable workspace, the manipulator has the specified 280 mm thick reachable hemispherical subshell 190 mm from the base. Using 100:1 geared MX-12W smart servos, the precion from the first three links is 0.0008 degrees, far less than the minimum specified joint angle precision of 0.025 degrees. Increased angular precision allows for greater imprecision in other physical features of the design. With the 100:1 gear ratio provided by the double pulley system, the manipulator can lift itself. The double pulley system is shown in \emph{Figure \ref{fig:drawn}} on Link 3. It does the aforementioned within the end-user’s budget of \$1000 and MEIOSIS’s budget of \$800.

\begin{figure}[htp]
  \center
  \includegraphics[width=.9\textwidth,frame]{drawn}
  \caption{Double Pulley Design Revision Drawing}
  \label{fig:drawn}
\end{figure}
\newpage
\emph{Figure \ref{fig:gantt}} shows a high-level Gantt chart for the Detail semester.
...
After spring break, 13 March, the Gantt chart allots one month for improvements to the design. While not specified features, adding object detection and 3D printable end-effector patterns would increase the manipulator’s educational value without increasing its cost. Alternatively, this month permits for several iterations of the manipulator to be printed either for design optimisation or to remediate unanticipated issues with meeting specifications.Since the pulleys are 3D printed several days are allocated to refining their tolerances to mesh with timing belts. Rather than printing in PLA, they may be printed in resin for increased tolerance precision. Further, the shafts will be 3D printed and may require an additional support to avoid the shear stress of a cantilever beam near joint 3 and the wrist differential. The remaining physical system is printed and tested concurrently with the software implementation.
